...
Quaternion Quaternion::operator*(float const &number) {
    return Quaternion(this->r*number, this->i*number, this->j*number, this->k*number);
}

Quaternion Quaternion::operator+(Quaternion const &quaternion) {
    return Quaternion(this->r + quaternion.r, this->i + quaternion.i, this->j + quaternion.j, this->k + quaternion.k);
}
